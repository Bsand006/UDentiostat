...
def startup_routine():
    ...
    board.analog[0].register_callback(a0_callback)
    board.analog[2].register_callback(a2_callback)
...
def start_exp(d9, normalized_start, data):
    """Initializes the writing pin and registers callbacks for analog pins.
...
def read_write(
    ...
    for x in steps_list:
        ...
        for _ in range(average):
            ...
            if a0_voltage is None or a2_current is None:
                continue

            real_voltage = (a0_voltage - voltage_offset) * -cf
            real_current = (a2_current - voltage_offset) * -cf / sr
            voltage_catcher += real_voltage
            current_catcher += real_current
